...
public class Cell {
    ...
    private ArrayList<PseudoAnimal>[] entities ;
    ...
    public synchronized void add(PseudoAnimal animal){
    ...
    public synchronized void remove(PseudoAnimal animal){
        int type = animal.getType();
    ...
    public synchronized void removeAll(int type){
        for(int i=countOfEachType[type-1]-1 ; i>=0 ; i--){
            PseudoAnimal animal = entities[type - 1].get(i) ;
            animal.kill("animal's attack");
        ...
        if(isBalanced())
            ...
            for(int i=0 ; i<r ; i++)
                for(int j=0 ; j<entities[i].size() ; j++){
                    PseudoAnimal animal = entities[i].get(j) ;
                    entities[i].remove(j) ;
                    countOfEachType[i]-- ;
                    animal.kill("low capacity") ;
                    if(isBalanced())
                        return;
                }
        }
